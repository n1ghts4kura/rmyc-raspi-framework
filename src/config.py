...
ENABLE_IMAGE_PREPROCESSING = True  # 已启用：gamma=0.8 压暗高光

# Gamma 校正值
# ⚠️ 使用前必读：
#   - 光照充足场景：gamma=1.0（无变化）或 0.9（轻微压暗）
#   - 光照不足场景：gamma=1.2-1.5（提亮）
#   - 修改后必须重新采集训练数据！
# 
# 性能优化：已使用 LUT 缓存，性能影响可忽略
IMAGE_PREPROCESSING_GAMMA = 0.8  # 实测最佳值：压暗高光，避免过曝

# 预处理记录（用于文档追溯）
# 采集日期：2025-10-12
# 采集工具：training/data_collector.py
# 硬件限制：树莓派摄像头不支持手动曝光
# 预处理方法：Gamma 校正（gamma=1.3）
