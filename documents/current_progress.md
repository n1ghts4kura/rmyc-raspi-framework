...
## 2025-10-11: 图像预处理策略设计

### 发现的新问题
- **相机硬件限制**: Raspberry Pi 摄像头无法通过 `cv2.CAP_PROP_BRIGHTNESS` 调整曝光
...
1. **Gamma 校正**: 压制高光区域
2. **LAB 色彩空间**: 独立处理亮度通道
3. **CLAHE**: 对比度受限自适应直方图均衡
4. **自适应策略**: 根据图像质量动态调整

**预期改进**:
- 检测精度: 65% → 78-85% (提升 15-25%)
- 置信度: 0.45-0.60 → 0.65-0.80
- 误检率: 15% → 7-10%
- 性能损失: 10-20% 帧率（可接受）

### 文档输出
- ✅ `documents/image_preprocessing_strategy.md` (14KB) - 详细技术分析
- ✅ `IMAGE_PREPROCESSING_GUIDE.md` - 快速实施指南

### 待实施
- [ ] 在 `src/config.py` 添加预处理配置
- [ ] 在 `src/recognizer.py` 实现 `_adaptive_preprocess()` 方法
- [ ] 测试验证效果
- [ ] 参数调优
