...
### 问题发现的契机
在完成目标选择器后，开发者运行 `test_annotation.py` 测试视觉识别系统的实时性能，准备为自瞄系统验证基础推理能力。**结果发现帧率异常低（3.82 FPS）**，远低于自瞄系统设计目标（15-25 FPS）。
...
#### 根因分析
通过创建诊断工具 `test_inference_loop.py` 测试三种策略：
