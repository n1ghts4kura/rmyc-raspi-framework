...
> 只关注**当前设计与职责边界**，不复盘历史与踩坑（详见 `aimassistant_journey.md`）。
...
> 详细推导与调参过程，可参考 `aimassistant_journey.md` 与性能相关的 journey 文档。
...
- 调用频率：
  - aimassistant 通常由自瞄技能周期性调用，频率与视觉推理 FPS 接近（目标为 4+ FPS，详见 `performance_journey.md`）。
- 性能假设：
...
> 若出现 FPS 掉到 1FPS 或明显卡顿的问题，应优先参考 `performance_journey.md`，
...
- `docs/journey/aimassistant_journey.md`
